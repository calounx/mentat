...
# Configure Nginx for CHOM application
configure_nginx() {
    log_step "Configuring Nginx for CHOM application"

    local app_domain="${APP_DOMAIN:-chom.arewel.com}"
    local app_root="/var/www/chom/current/public"

    # Create nginx configuration (HTTP initially, HTTPS added by setup-ssl.sh later)
    sudo tee /etc/nginx/sites-available/chom > /dev/null <<NGINX_CONF
# CHOM Laravel Application
# Generated by prepare-landsraad.sh

upstream php-fpm-chom {
    server unix:/run/php/php8.2-fpm-chom.sock;
}
...
NGINX_CONF
    ...
    sudo ln -sf /etc/nginx/sites-available/chom /etc/nginx/sites-enabled/chom
...
setup_ssl_certificates() {
    ...
    local app_domain="${APP_DOMAIN:-chom.arewel.com}"
    local ssl_email="${SSL_EMAIL:-admin@arewel.com}"
    ...
    if [[ -f "/etc/letsencrypt/live/${app_domain}/fullchain.pem" ]]; then
        log_success "SSL certificate already exists for ${app_domain}"
        # Still update nginx config to use HTTPS
        configure_nginx_https
        return 0
    fi

    # Obtain certificate
    log_info "Obtaining SSL certificate for ${app_domain}"
    if sudo certbot --nginx -d "${app_domain}" --non-interactive --agree-tos --email "${ssl_email}" --redirect; then
        log_success "SSL certificate obtained for ${app_domain}"
    else
        log_warning "SSL certificate setup failed - continuing with HTTP only"
        log_info "You can manually run: sudo certbot --nginx -d ${app_domain}"
        return 0
    fi
}
...
configure_nginx_https() {
    log_step "Configuring Nginx with HTTPS"
    ...
    sudo tee /etc/nginx/sites-available/chom > /dev/null <<NGINX_CONF
# CHOM Laravel Application with HTTPS
# Generated by prepare-landsraad.sh
...
NGINX_CONF
    ...
    if sudo nginx -t; then
        sudo systemctl reload nginx
        log_success "Nginx configured with HTTPS for ${app_domain}"
    else
...
main() {
    ...
    # Deploy exporters for all detected services
    ...
    log_info "Next steps:"
    log_info "  1. Deploy the application with deploy-application.sh"
    log_info "  2. Run health checks to verify deployment"
}
