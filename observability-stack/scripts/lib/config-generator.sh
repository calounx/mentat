...
aggregate_alert_rules() {
    ...
    # Copy rules from each enabled module
    while IFS= read -r module; do
...
provision_dashboards() {
    ...
    # Copy dashboards from each enabled module
    while IFS= read -r module; do
...
generate_all_configs() {
    ...
    local temp_file
    temp_file=$(mktemp)
    echo "$prom_config" > "$temp_file"

    if command -v promtool &>/dev/null; then
        if promtool check config "$temp_file" >/dev/null 2>&1; then
            cat "$temp_file" > "$prometheus_config"
            log_success "Prometheus configuration generated: $prometheus_config"
        else
            log_error "Generated Prometheus config is invalid!"
            promtool check config "$temp_file"
            rm -f "$temp_file"
            return 1
        fi
    else
