...
config_file = "$config_file".replace("$config_file", sys.argv[1] if len(sys.argv) > 1 else "/etc/loki/loki-config.yaml")

# Use proper config file path
...
install_binary() {
    ...
    if type download_and_verify &>/dev/null; then
        ...
        wget -q "$download_url" || {
            log_error "Failed to download Loki binary"
            return 1
        }
