...
upgrade_alloy() {
    ...
    if systemctl list-units --type=service --all | grep -q "^[[:space:]]*alloy.service" 2>/dev/null; then
        if type stop_and_verify_service &>/dev/null; then
            ...
            local wait_count=0
            while pgrep -f "$alloy_binary" >/dev/null 2>&1 && [[ $wait_count -lt 30 ]]; do
                sleep 1
                ((wait_count++))
            done
            ...
            while [[ $wait_count -lt 30 ]]; do
                if ! lsof "$alloy_binary" &>/dev/null 2>&1; then
            ...
            # Final verification
            if lsof "$alloy_binary" &>/dev/null 2>&1; then
                log_error "File lock still held on $alloy_binary - cannot replace binary"
                return 1
            fi
        fi
    fi
